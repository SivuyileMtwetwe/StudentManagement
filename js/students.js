...
function displayStudents(
  ...
  let displayedStudents = searchQuery
  ...
  contentDiv.innerHTML = `
        ...
        <select id="sortSelect">
            ...
            <option value="age" ${
              sortField === "age" ? "selected" : ""
            }>Sort by Age</option>
            <option value="class" ${
              sortField === "class" ? "selected" : ""
            }>Sort by Class</option>
        </select>
        ...
        <div class="pagination">
            ${Array.from({ length: totalPages }, (_, i) => i + 1)
              .map(
                (num) =>
                  `<button onclick="displayStudents(students, '${searchQuery}', '${sortField}', ${sortAscending}, ${num})" ${
                    num === page ? "disabled" : ""
                  }>${num}</button>`
              )
              .join("")}
  ...
  document
    ...
    .addEventListener("click", importStudentsFromCSV);
  document
    .getElementById("deleteAllBtn")
    .addEventListener("click", deleteAllStudents);
  ...
  document
    ...
    .addEventListener("click", () => exportToCSV(students));
}
...
function exportToCSV(students) {
  const csvContent =
    "data:text/csv;charset=utf-8,Name,Age,Class,Subjects,Scores\n" +
    students
      .map(
        (s) =>
          `${s.name},${s.age},${s.class},${s.performance
            .map((p) => p.subject)
            .join("|")},${s.performance.map((p) => p.score).join("|")}`
      )
      .join("\n");
